...
interface JobCardProps {
  ...
  onUnlockContact?: (job: FreelanceJob) => void;
...
export const JobCard: React.FC<JobCardProps> = ({
  ...
  const requiresUnlock = Boolean(job.requiresMissionToUnlockContact) && !isContactUnlocked;
  ...
  const handleDirectWhatsAppContact = () => {
    if (requiresUnlock && onUnlockContact) {
      onUnlockContact(job);
      return;
    }
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
          ...
          {/* Gender Requirement Highlight */}
          {(job.genderRequirement === 'homens' || job.role.toUpperCase().includes('HOMENS')) && (
            <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black bg-amber-500/20 text-amber-300 border border-amber-500/50">
              <Users className="w-3 h-3 text-amber-400" />
              <span>APENAS HOMENS</span>
            </span>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-4 text-xs text-slate-300 bg-slate-950/70 p-3.5 rounded-xl border border-slate-800/80">
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            <span className="font-bold text-white">📅 Datas & Horário:</span>
            <div className="text-slate-300">
              {job.date} • <span className="font-semibold text-emerald-300">Das {job.startTime} às {job.endTime}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            <span className="font-bold text-white">Vestimenta & Regras:</span>
            <div className="text-slate-300 font-medium line-clamp-2" title={job.dressCode}>
              {job.dressCode}
            </div>
          </div>
        </div>
          ...
          {job.benefits ? (
            ...
            <>
              ...
              <div>
                ...
                <div className="text-slate-300">
                  {requiresUnlock ? 'Protegido por Missão TikTok' : `${job.contactPhone} (${job.contactName || 'Coordenação'})`}
                </div>
              </div>
            </>
          )}
      ...
      {/* Sponsor Mission Contact Banner for Locked Jobs */}
      {requiresUnlock && (
        <div className="mb-3 p-3 rounded-xl bg-gradient-to-r from-pink-950/40 via-purple-950/30 to-slate-900 border border-pink-500/40 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-pink-500/20 text-pink-400 border border-pink-500/40 flex items-center justify-center shrink-0">
              <Lock className="w-4 h-4" />
            </div>
            <div>
              <div className="text-xs font-black text-white flex items-center gap-1.5">
                <span>Contato Protegido por Missão TikTok</span>
                <span className="text-[10px] px-1.5 py-0.2 rounded bg-pink-500/20 text-pink-300">Patrocinador</span>
              </div>
              <p className="text-[11px] text-slate-400">
                Acesse a missão rápida do TikTok para liberar o WhatsApp privado da vaga.
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={() => onUnlockContact && onUnlockContact(job)}
            className="w-full sm:w-auto px-3.5 py-1.5 rounded-lg bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-400 hover:to-rose-400 text-white text-xs font-black shrink-0 transition shadow-md shadow-pink-500/20"
          >
            Liberar Contato Agora
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 pt-3 border-t border-slate-800/80">
        ...
        {/* Contact direct link or unlock button */}
        {requiresUnlock ? (
          <button
            onClick={() => onUnlockContact && onUnlockContact(job)}
            className="flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl text-xs font-black text-white bg-gradient-to-r from-pink-600 via-rose-600 to-amber-600 hover:from-pink-500 hover:to-amber-500 shadow-md shadow-pink-600/20 transition transform active:scale-95 animate-pulse"
          >
            <Lock className="w-3.5 h-3.5" />
            <span>Liberar Contato no TikTok</span>
          </button>
        ) : (
          <button
